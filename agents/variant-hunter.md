...
You receive:
- **Finding path**: `piolium/findings-draft/<phase>-<NNN>-<slug>.md`
...
- Run the CodeQL query against `piolium/codeql-artifacts/db/`
...
codeql query run \
  ...
  --output=piolium/tmp/variant.bqrs \
  -- piolium/codeql-queries/variant-<slug>.ql
codeql bqrs decode --format=json piolium/tmp/variant.bqrs
```
...
Check `piolium/chamber-workspace/*/variant-candidates/` for pre-identified candidates
...
Write each confirmed variant to `piolium/findings-draft/p12-<NNN>-<slug>.md` using this template:
