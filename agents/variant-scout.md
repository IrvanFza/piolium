...
1. Read `piolium/chamber-workspace/<chamber-id>/debate.md` after each round marker appears (`## Round N`). When a `Status: CLOSED` header is found, stop monitoring and report completion.
...
codeql query run \
  ...
  --output=piolium/tmp/variant-search.bqrs \
  -- piolium/codeql-queries/on-demand-variant-<slug>.ql
codeql bqrs decode --format=json piolium/tmp/variant-search.bqrs
```
...
```
piolium/chamber-workspace/<chamber-id>/variant-candidates/<slug>.md
```
...
`piolium/chamber-workspace/<chamber-id>/variant-candidates/` for Phase 12 variant analysis
