...
- **Skip completed phases**: if the expected KB sections already exist and neither the codebase nor
  ...
  skipped.
- **Re-audit as KB update**: for re-audits, `piolium/attack-surface/knowledge-base-report.md` from the previous
...
- **Partial resumption (Phases 7-9)**: if `piolium/findings-draft/` already contains draft files,
  resume from the existing drafts rather than starting fresh.
- **State recording**: `piolium/audit-state.json` is append-only. Before starting a new audit,
...
1. **Security folder dirtiness check**: Run `git status piolium/` to determine if `piolium/` has
...
3. **Worktree isolation**: If BOTH conditions hold — `piolium/` is dirty AND another agent is
   ...
   Run the entire audit from inside `$AUDIT_WT`. All `piolium/` output is written to the
...
Populate the `## CodeQL Structural Analysis` section of `piolium/attack-surface/knowledge-base-report.md`
...
- Run GitHub Actions review with `agentic-actions-auditor` when `.github/workflows/` exists; write results to the `## GitHub Actions Audit` section of `piolium/attack-surface/knowledge-base-report.md`.
...
- Store all dynamically generated custom rules in `piolium/codeql-queries/` and `piolium/semgrep-rules/`.
- Document exactly what custom rules were created, why they match the Phase 3 architecture, and their execution results in the `## Static Analysis Summary` section of `piolium/attack-surface/knowledge-base-report.md`.
...
1. Read the `## Domain Attack Research` section of `piolium/attack-surface/knowledge-base-report.md` first —
...
Write all findings to the `## Spec Gap Analysis` section of `piolium/attack-surface/knowledge-base-report.md`.
...
Phase 10 uses a **Review Chamber** multi-agent debate system. Instead of a single deep-auditor
...
1. Read `## High-Risk DFD Slices` and `## High-Risk CFD Slices` from `piolium/attack-surface/knowledge-base-report.md`
...
- Finding drafts: `piolium/findings-draft/p7-<NNN>-<slug>.md` (Medium+ only, Low dropped)
- Debate transcripts: `piolium/chamber-workspace/<chamber-id>/debate.md` (audit artifact)
- Variant candidates: `piolium/chamber-workspace/<chamber-id>/variant-candidates/` (for Phase 12)
- Pattern registry: `piolium/attack-pattern-registry.json` (for Phases 8, 9)
...
- `piolium/chamber-workspace/*/variant-candidates/` for pre-identified candidates from Variant
  Scouts
- `piolium/codeql-artifacts/entry-points.json` and `sinks.json` for structurally similar
...
- On-demand QL queries against `piolium/codeql-artifacts/db/` for AST-level structural matches

**Incremental persistence**: Write each confirmed variant immediately to `piolium/findings-draft/p9-<NNN>-<slug>.md` using the finding draft template. Only create drafts for variants rated Medium or higher.

**Database cleanup**: After all variant queries complete, delete the CodeQL database:

```bash
rm -rf piolium/codeql-artifacts/db/
```

The extracted JSON and markdown summaries in `piolium/codeql-artifacts/` are retained as
...
6. Output all technical details and the PoC script for each single bug in its own dedicated subfolder under `piolium/findings/<Cn|Hn|Mn>-<bug-name>/`.
7. **CRITICAL/HIGH real-environment mandate**: For every CRITICAL or HIGH finding, real-environment PoC execution is required. Reuse the Stage 2 adversarial environment if available; otherwise provision a new one. Capture evidence in `piolium/findings/<ID>-<slug>/evidence/`. Annotate `PoC-Status: executed | theoretical | blocked` in the finding. A `theoretical` or `blocked` status requires a `PoC-Block-Reason:` line.
...
1. **Finding ID cross-reference**: Every finding ID referenced in `piolium/final-audit-report.md` must correspond to a directory in `piolium/findings/`.
2. **KB section completeness**: `piolium/attack-surface/knowledge-base-report.md` must contain all phase-labelled sections. Sections labelled Phase 1-6 must be non-empty. Phase 10 Addendum must exist after Phase 10.
3. **Orphan detection**: Files present in `piolium/` but not referenced by the KB or `final-audit-report.md` are flagged as orphans.
...
5. **Findings-draft promotion**: Before final cleanup, `piolium/findings-draft/` should contain no files with verdict `VALID` that are missing a corresponding directory in `piolium/findings/`.
6. **CodeQL artifact completeness**: After Phase 4, `piolium/codeql-artifacts/entry-points.json`, `sinks.json`, `call-graph-slices.json`, and `flow-paths-all-severities.md` must all exist.
7. **No Low severity leakage**: `piolium/findings/` must contain no directory with an `L`-prefixed ID, and `piolium/final-audit-report.md` must contain no `LOW` entry in the findings table.
8. **No stale separate reports**: `piolium/` must not contain `advisory-hunter-report.md`, `bypass-analysis-report.md`, `threat-model-report.md`, `attack-surface-report.md`, `static-analysis-report.md`, `actions-audit-report.md`, `spec-gaps-report.md`, or `final-findings-report.md`. These have been consolidated into `knowledge-base-report.md`.
